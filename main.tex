...
Both astronomers and astrophysicists contribute to and criticize the scientific literature. One could object to my claim that \textsl{Gaia} was not built by astronomers by saying that any employee of EADS who works on \textsl{Gaia} is obviously an astronomer:
...
Astronomers, in the case of \textsl{Gaia}, are just end users; end users of curated, calibrated data, delivered by a combination of the (secret) spacecraft and the (absolutely great, professional) DPAC.
...
Not all of astrphysics is data science, not by a long shot!
...
When we employ a graduate student to perform some work, it absolutely must be \emph{because the graduate student will benefit} from that work, not because that work needs to get done.
...
We train PhD students not to amplify our own research programs, but to create opportunities, and specifically opportunities for them.
...
But invdividual things that PhD students are doing are way less important than those PhD students themselves.}
...
However, I believe that low-cost entities that just provide a ``refereeing layer'' but don't actually preserve the literature, and don't actually publish content, don't contribute much, since they are neither a disseminator nor a preserver of literature.
...
After all, graduate students aren't \emph{labor} to be \emph{efficiently deployed}.
...
The argument presented here does not make assumptions about medium-term or long-term futures.}
...
One thing I have not addressed at all in this \documentname{} is the tremendous environmental impact of the training and execution of the LLMs.
The numbers are staggering.
It is important to remember that the human practice of astrophysics, at least in its current form, is also very damaging to the environment \cite{low-carbon, imperative}.}).
Under this policy, the vast, vast majority of astrophysics would be done by machines, and we (as a community of humans) would participate primarily through reading, vetting, and discussing the novel astrophysical results produced by the LLMs.
...
HOGG: THINGS TO SAY:
It's efficient, and (maybe) rigorous. It also serves all of the public-interest, and public-education, and contribution-to-knowledge aspects of the subject.
It probably also serves the development and military uses of astrophysics, provided that the robots can propose and plan truly new methods and experiments.

HOGG:
On the other hand, most of the benefits of astrophysics are about people:
Educating them, training them, uniting them, satisfying them.
And, according to the arguments above, it is impossible to ``be an astrophysicist'' without ``doing astrophysics.''
So if we just let them cook, there would be no more astrophysicists.
Then Universities would be diminished, then education, etc.

\paragraph{Ban and punish:}
The other possible extreme policy proposal is that we \emph{fully outlaw} the LLMs, and do not permit anyone in astrophysics to use them for anything other than the most menial of tasks (like summarizing documentation), and enforce negative consequences for people or projects that used them.
