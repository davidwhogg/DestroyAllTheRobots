...
Of course it is important to remember that the human practice of astrophysics, at least in its current form, is also very damaging to the environment \cite{low-carbon, imperative}.\label{note:environment}})
...
A comment to make about let-them-cook is that it would absolutely need to be associated with strong conventions or policies about transparency, disclosure, and (possibly) reproducibility.
...
For this very \documentname, I used LLMs to learn some details about BibTeX formatting, and also get a summary of some ideas in post-structuralist literary criticism (see below).
...
ALSO work showing the increase in slop on arXiv.
Maybe this should go into the introduction where we introduce \cite{denario}.
