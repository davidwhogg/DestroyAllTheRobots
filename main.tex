...
% ----------------------------
% - Look at notes from AGG Community Meeting from 2026-01-26 (maybe, or the week before?) and see who I want to quote.
...
% - Meek and Greg thought that I haven't said enough about what the LLMs DO or ARE.
% - Figure out how to cite perspectives by Ting and Roberto.
% - Huang comments (email 2025-02-04), especially papers with ChatGPT as an author and the point about copyright agreements.
...
I list and discuss every possible benefit that astrophysics can be seen as bringing to us, and to science, and to universities, and to the world.
I conclude with a discussion of possible policy recommendations, none of which I endorse.
...
\emph{HOGG (note to self): For context, LLMs are ... Describe what LLMs do and why they are not AIs.}
...
There is another change happening in astrophysics, which is happening on a much longer time scale, but which is relevant:
...
\emph{HOGG (note to self): Is there anything I can cite where an LLM HAS BEEN MADE a co-author?}
...
I'll return to that point at the very end.})
...
One idea or rule of thumb is perhaps that we should or could interact with an LLM only in the same ways that we would interact with a non-coauthor colleague.\footnote{This suggestion came to me from Adrian Price-Whelan (Flatiron).}
...
You might ask your colleague to help speed up your code, but you wouldn't have your colleague write your code.
I like these ideas; maybe they could evolve into conventions or community-supported best practices.

The only strong positive position I can put forward here is that transparency and disclosure are going to be important parts of whatever we do.
...
\emph{HOGG (note to self): My position is that LLMs are pivoters of documentation. Use post-structuralism as the example.}

\emph{HOGG (note to self): GREG asks: What would an LLM have to do to earn your trust? Is it even possible for something that has no accountability, cannot take responsability or suffer consequences (career setbacks, humiliation) to be trustworthy? How is an LLM different from a corporation?}
...
HOGG (note to self): Andy says: \textsl{For your final paragraph I think you should end on “why did I write this white paper”, which might include something about how there is a very diverse set of perspectives on LLMs in astronomy (just even within your circle). People have very strong and disparate views, and ultimately they are probably arguing about the wrong question of HOW we do astronomy when they should be asking WHY do we do astronomy.} I agree with that!
