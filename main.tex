...
% - On code, and use for coding, cite Bloom blog post? Where to put this?
% - look at paragraph breaks?
% - explicitly connect the effective altruism point to the instrumental view of astrophysicists.
% - The idea that the "people" point isn't exclusive: We would also treat aliens or intelligent machines with respect. (Gautam point)
% - The point that if people do science with closed source code, something is missing; maybe that maens that code *is* literature?
% - The idea that, right now, it is all terrible slop. So this all is really a fantasy. Related: The idea that if the LLMs get way, way better, then the LLMs will also change all sorts of things, and astrophysics changes won't matter in that context.
% - Connect the correctness and rigor point with the trust point.
% - Make it clear that this is about Contemporary WESTERN astrophysics
% - Related: Ancient and non-western communities used astronomy to schedule the harvest and schedule cultural events, also for navigation!! Cite things.
% - Comment on the fact that it will all be SLOP. No matter what.
% - Extensive KSF comments in email dated 2026-01-20 (or maybe 19).
% - Cite blog posts by Wes and Bloom re (code) languages of LLMs.
% - cite this: https://datasociety.net/points/on-arxiv-an-influx-of-ai-slop-pits-surface-against-substance/
% - Contardo: Breaks the collective / community aspects of Astrophysics. Only three actors.
% - Do we want a header on all pages?
% - Is it ABSOLUTELY CLEAR why I object to let-them-cook?
% - Are we clear enough (about slop) that the models would have to get way way better to be as robust and creative and interesting and reliable and responsible as humans?
% - Meek thought that I haven't said enough about what the LLMs DO or ARE.
...
% [GREG] There's a dissonance between "possible" and the lack of endorsement. I'm wondering why they don't warrant your endorsement but they do warrant the time we'll spend presenting/reading about them.
...
% [GREG] I agree with Meek (you mentioned he asked for more on LLMs). A short para on what an LLM is/does. That would be a good place to explain your objection to the term genAI. I read "I really don't like that" and wonder why and you never tell me. This makes me sad ;-) I'd move the example (denario) after the line "LLMs can do data science." I see the flow as (0) this \docname is inspired by the rapid changes taking place simultaneously the area of artificial intelligence and the way astrophysics projects are done/run(?) (1) In the last two years, the capacities of LLM have grown beyond belief. What is an LLM. Why the rapid change. Briefly, why not "genAI". Wouldn't matter more to astrophysicist than anyone else were it not for change in projects (2) In the last several decades, changes in project: professionalization of etc. Data science. (3) This matters because LLMs can do data science. Denario example, etc.
...
% [GREG] This section on the professionalization of astrophysics is one of my favorites. Very strong.
There is another change happening in astrophysics, however, which is happening on a much longer time scale, but which is relevant:
...
% [GREG] "facts", "we can all agree" vs "many... disagree". Is that because those astrophysicists are wrong? 
...
% [GREG] I had to look up "categorical imperative". 

I have heard it said, more than once, that an LLM can do some task ``better than a graduate student.''
That language makes me uncomfortable, because it is taking an extremely \emph{instrumental} view of graduate students.
% [GREG] I feel the discomfort, but I'm left wondering whether it is nonetheless true. LLM could be less error prone, impossible to discourage, undistracted, have a better grasp of fundamentals, are more articulate, better at taking criticism, etc. Long division: saying any computer is better at doing long division than any human isn't considered a slight to humans. But saying "any computer is better at long division that an eight-year-old" gets closer to the discomfort of the LLM vs grad student comparison because we understand that it is not the purpose of an eight-year-old to be good at long division. Their purpose, arguably any human's purpose, is to play and grow and learn and be and feel. What an LLM can't do "better than a grad student" is those things and we'd never expect it to. Long division and astrophysics are some of things we can spend our time on while we're here. You point out later all the ways that astrophysics has more going for it that long division.
Are graduate students in our business \emph{to do work}?
% [GREG] Maybe don't refer to it as "our business". To me, a business is an entity that produces good and services in exchange for some kind of benefit, usually payment. In the "business of science", it may be that graduate students are indeed cogs, instruments. I agree with you that we must reject the instrumental view of the world and embrace a more noble, humanistic view and focus on nurturing each other, especially as machines become increasingly good at things that we have always believed were part of what made us special BTW I like that you talk about other intelligences later.
...
% [GREG] Also individuals can *choose* to sacrifice themselves in pursuit of those accomplishments. Many people have; most of them are not remembered. It's quite different to secretly think it's ok to sacrifice grad students "for the greater good" while at the same time leading them to believe that you're acting in their best interests. I really like what you say below about Utilitarianism vs. capitalism.
}
...
One weird comment to make here is that, when I say that ``people'' are always the ends, I don't mean to be human-centric.
...
% [GREG] Do LLMs embody and execute our ideas and beliefs about... everything in their training data?
...
% [GREG] Off-off-topic: if all the literature is lost but the LLMs trained on the literature somehow survive, what exactly has survived? It's crazy how these models work and generate uncanny output but try asking Claude to list the 20 most influential papers in astrophysics in the last 50 years: author list, title and abstract as nearly as possible. Not only will the model not push back on the idea of "most influential" but, as you know, there will be all kinds of errors in the result. Of course, being able to faithfully reconstruct even a single important paper from memory is not a requirement we have of people either. So I don't know how to feel about all that.
}
...
% [GREG] Yeah, somebody really should write that book... There can be a chapter about mentoring grad students in the age of "smart machines".
}
...
% [GREG] Do scientists never complain about having to write papers (more papers, more highly cited papers)? I'm not saying complaining about it is good for your health but there is real pressure to produce papers. It makes sense because, as you say, if you're not contributing to the astro lit, you aren't much of an astrophysicist.
...
% [GREG] Did you add this last sentence recently. I don't remember seeing this "concession" on my first read-through.
...
Things might get weirdly different if LLMs obtained very long-term memories.}
HOGG IS THERE anything I can cite where an LLM HAS BEEN MADE a co-author?
% [GREG] When I read this, I thought about the earlier passage about the professionalization of data gathering and how there's a lot you don't know about how those data sets are generated. When you use data from one of these big projects, who is responsible for the quality of the data? Why do you trust them? (Ah, I just scrolled down and here comes trust! Yeah!)
...
% [GREG] What would an LLM have to do to earn your trust? Is it even possible for something that has no accountability, cannot take responsability or suffer consequences (career setbacks, humiliation) to be trustworthy? How is an LLM different from a corporation?
...
% [GREG] The full-cost accounting of LLM effort vs human effort is, to say the least, challenging. So how can we decide which approach is a better use of resources? I guess your order of magnitude points later are part of the argument pro LLM.
...
Thus we check or ``confirm'' most measurements in astrophysics primarily or partially \emph{by inspection}, inspection of the data and code that were taken and used in the generating the analysis and conclusions.
...
% [GREG] You're really teasing this one. I'm thinking "when will he dig into this"?
...
% [GREG] At last! The most teased "agreement".
...
% [GREG] Isn't a lot of justification (grant proposals in bio) pretty performative? If you don't draw a line to the right-edge of whatever left-edge thing you're doing, no funding. Like aspirational clinical value.
None of astrophysics is justified in these right-edge terms.
% [GREG] Exactly. In astro, you don't have to pretend.
...
but I do have to admit that the work we do on instrument calibration and the detection of faint, moving objects (my own contribution here being \cite{undetectable}) could deliver enormous, direct benefits for humanity some day.
...
% [GREG] Do you know if he meant "look at this tricky thing we're doing sort of for the sake of doing it despite all our flaws and tendancy to do dumb/violent things; there's hope for us" or "we keep learning more about our universe; who knows what we'll learn"
...
% [GREG] Exactly. The hope is a function of the endeavour not the outcome. 
...
% [GREG] Again I feel this section intro is better than the first version I read. Maybe I'm imagining it.
...
% [GREG] The policies are both extreme and simple?
...
% [GREG] my point about using resources efficiently
...
If we take the ratio of minutes to months, or the ratio of the time for an LLM to completely do a scientific project to the same but for a human, we get something like a factor of $10^5$.
...
This point was made very strongly to me by Natalie B. Hogg \cite{natalie}.
Of course, since we all learn what we know and how to explain it by reading and talking to others, maybe even we humans are all plagiarists?
...
But transparency is also required for \emph{reproducibility}, and one very disturbing thing about the current LLMs is that they are not even close to reproducible.
...
% [GREG] How is the non-reproducibility you're talking how here different from the non-reproducibility in the "Correctness and rigor" section. There, it seemed to be something you could live with?
...
A related but different point---about the computer languages in which LLMs will write code in the future---is being discussed in the vibe-coding arena \cite{wes, bloom}.}
...
The only positive idea I have here is that transparency and disclosure are going to be important parts of whatever we do.
...
For this very \documentname, I used LLMs to learn some details about BibTeX formatting, and also get a summary of some ideas in animal rights (see above) and post-structuralist literary criticism (see below).

HOGG: My position is that LLMs are pivoters of documentation. Use post-structuralism as the example.
...
This makes sense because it is hard to assess the quality and impact of scientific work when that work is young and relatively untested; citation counts serve as proxy.
...
Astronomers like to hate on astrology, but astrological signs (when they are properly maintained in terms of the Sun's actual angular location among the constellations) can be seen as a coordinate-free, universally accessible, and reproducible record of the time of year, divorced from state power.}
...
Perhaps meaning is what is transmitted from the author's knowledge to the reader's knowledge.
If the LLMs don't ``know'' anything, then, in this view, the documents they ``write'' might end up being meaningless.

HOGG: IS THERE A FINAL CALL TO ACTION? I DON'T THINK SO.
