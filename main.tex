...
% ----------------------------
% - create a blogpost bibtex format
% - comment on my own use of LLMs in writing this paper: Bibtex help, and post-structuralism, say.
...
% - Extensive Meek comments in email dated 2026-01-19.
...
% - I need a translator on Kant, and maybe also Barthes?
...
None of these are responsible for any of the claims presented here.
...
I am not certain that it is \emph{intelligence}, precisely, that makes an entity deserving of inclusion in the categorical imperative, but whatever it is, I doubt it is ``membership in the human species.''}
...
That is, it isn't really correct, if you use an LLM, to give that LLM co-authorship on your paper, any more than it is to give astropy \cite{astropy} or copilot \cite{copilot} or the Mac operating system coauthorship.
...
There are extreme cases where very, very bad research practices render papers unciteable.
...
If we implement this full-acceptance (``let-them-cook'') policy, astrophysics changes dramatically, but it is consistent with some of our points of agreement (listed in \secref{sec:facts}), and it does deliver many of the key benefits of astrophysics (listed in \secref{sec:benefits}).
...
HOGG: A place here to ask: Are the trust issues with LLMs far worse than with other tools, like say astropy \cite{astropy}?
Right bow the answer to this is hell yeah.
Interesting whether anything changes if the LLMs became way more trustworthy?
...
A comment to make about let-them-cook is that it would absolutely need to be associated with strong conventions or policies about transparency, and (possibly) reproducibility.
...
But transparency is also required for reproducibility, and one very disturbing thing about the current LLMs is that they are not even close to reproducible.
...
then there is no reason to expect that the scientific literature would or should be written in human-readable languages.
In this future, presumably ``reading the literature'' means having an LLM search and translate it for you.
...
It artificially restricts what astrophysicists---and especially early-career astrophyisicists---can do.
...
HOGG:
Is there room to talk about Price-Whelan's point that our (effective) focus on productivity or production or metrics might be disastrous here.
Which is better, a student who produces one great paper, or a sudent who produces eight good papers, in their PhD.
It depends! But since the greatness of a paper is not a visible property (and especially not when the paper is a new contribution, untested and uncriticized), there are usually more job opportunites for the one who produced eight.

HOGG: Maybe cite this from Stumm \url{https://x.com/ahall_research/status/2007603340939800664}
